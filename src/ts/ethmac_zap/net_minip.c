...
static int arp_resolve(uint32_t ip_be, uint8_t out_mac[6], unsigned timeout_iter){
    // Send ARP who-has
    {
        uint8_t pkt[64];
        struct eth_hdr* eth = (struct eth_hdr*)pkt;
    ...
    // Wait for ARP reply
    for (unsigned t=0; t<timeout_iter; ++t){
        unsigned len = 0;
        if (eth_rx_poll(rx_buf, &len) == 1 && len >= sizeof(struct eth_hdr)+sizeof(struct arp_pkt)){
            struct eth_hdr* eth = (struct eth_hdr*)rx_buf;
            if (ntohs(eth->type) == ETH_P_ARP){
                struct arp_pkt* arp = (struct arp_pkt*)(rx_buf + sizeof(*eth));
                if (ntohs(arp->oper) == 2 /*reply*/ && arp->tpa == htonl(MY_IP) && arp->spa == ip_be){
...
// ---------------- UDP Sender ----------------
int udp_send_to_ip(uint32_t dst_ip_be, uint16_t dst_port, const void* payload, unsigned plen)
{
    uint8_t dst_mac[6];
    if (arp_resolve(dst_ip_be, dst_mac, /*timeout_iter*/ 20000) != 0){
        uart_puts("arp: resolve failed\r\n");
        return -1;
    }
    ...
    uint8_t*        data= (uint8_t*)udp + sizeof(*udp);
    ...
    // IP
    const uint16_t ulen = sizeof(*udp) + plen;
    const uint16_t iplen= sizeof(*ip) + ulen;
    ...
    // UDP
    udp->sport = htons(12345);            // pick
    udp->dport = htons(dst_port);
    udp->len   = htons(ulen);
    udp->csum  = 0;                       // compute below

    // Payload
    for (unsigned i=0;i<plen;i++) data[i] = ((const uint8_t*)payload)[i];
    ...
    uint32_t sum = 0;
    // pseudo-header: src/dst/proto/udp_len
    sum = csum16_add(sum, &ip->saddr, 4);
    sum = csum16_add(sum, &ip->daddr, 4);
    uint16_t ph = htons((uint16_t)IPPROTO_UDP);
    sum = csum16_add(sum, &ph, 2);
    uint16_t ulen_be = htons(ulen);
    sum = csum16_add(sum, &ulen_be, 2);
    // UDP hdr + data
    sum = csum16_add(sum, udp, ulen);
    udp->csum = csum16_finalize(sum);
    if (udp->csum == 0) udp->csum = 0xFFFF; // RFC says 0 == 0xFFFF over wire

    return eth_send(pkt, sizeof(*eth) + iplen);
}
...
void net_poll(void){
    unsigned len = 0;
    while (eth_rx_poll(rx_buf, &len) == 1){
