...
there:
    ...
    ldr r10, =4001              /* MMU only. Change this literal to 4101 for MMU+caches. */
...
L1_IDENTITY_UNCACHED_LOOP:
    orr r2, r1, #0x02          /* section descriptor, C/B clear */
    str r2, [r0], #4
    ...
    /* --------------------------------------------------------------------- */
    /* Force all assembler literal-pool constants used above to be emitted
     * BEFORE MMU/cache enable.  After the following mcr, there must be no
     * ldr reg, =constant and no ldr sp, [pc,...] literal load.
     */
    .ltorg
